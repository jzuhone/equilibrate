...
class GaussianRandomField(object):
    def __init__(self, left_edge, right_edge, ddims, l_min, l_max,
                 ...
                 r2=None, A1=None, A2=None, prng=np.random):
        ...
        ax = np.fft.ifftn(sigma*v[0,:,:,:]).real
        ay = np.fft.ifftn(sigma*v[1,:,:,:]).real
        az = np.fft.ifftn(sigma*v[2,:,:,:]).real
        ...
        self.data["vector_x"] = YTArray(np.real(np.fft.ifftn(axk)), "%s*kpc" % field_units)
        self.data["vector_y"] = YTArray(np.real(np.fft.ifftn(ayk)), "%s*kpc" % field_units)
        self.data["vector_z"] = YTArray(np.real(np.fft.ifftn(azk)), "%s*kpc" % field_units)
