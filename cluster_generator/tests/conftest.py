...
def pytest_addoption(parser):
    ...
    parser.addoption("--gravity", help="The gravity to use.")


def pytest_collection_modifyitems(session, config, items):
    _module_order = ["cluster_generator.tests." + i for i in
                     ["test_particles", "test_profile", "test_gravity", "test_ics"]]
    _doc_test, its = [it for it in items if isinstance(it, pytest.DoctestItem)], [it for it in items if
                                                                                  not isinstance(it,
                                                                                                 pytest.DoctestItem)]
    ...
    for module in _module_order:
        sorted_items = [it for it in sorted_items if module_mapping[it] != module] + [it for it in sorted_items if
                                                                                      module_mapping[it] == module]
